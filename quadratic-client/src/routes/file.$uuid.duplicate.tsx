...
export const Component = () => {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (isOverLimit && !isPaidPlan) {
      showFileLimitDialog(maxEditableFiles ?? 3, selectedTeamUuid, () => doDuplicate(selectedTeamUuid));
      return;
    }
  ...
  useMemo(() => {
    if (singleTeamUuid) {
      ...
      apiClient.teams.fileLimit(singleTeamUuid, true).then(({ maxEditableFiles }) => {
        showFileLimitDialog(maxEditableFiles ?? 3, singleTeamUuid, () => doDuplicate(singleTeamUuid));
      });
    }
