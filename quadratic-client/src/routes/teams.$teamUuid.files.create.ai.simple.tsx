...
const DEFAULT_SUGGESTIONS: SuggestedPrompt[] = [
  {
    ...
    description: 'Build models for analysis',
  ...
  },
  {
    ...
    prompt: 'Retrieve the last 100 days of closing prices for Apple and chart on candlestick chart with TA indicators',
  },
];
...
export const Component = () => {
  ...
  const handleFileUpload = async (fileTypes: string[]) => {
    try {
      ...
      if (files.length > 0) {
        ...
        trackEvent('[StartWithAISimple].addFile', {
  ...
  const handleSubmit = useCallback(
    async (promptText: string) => {
      ...
      trackEvent('[StartWithAISimple].submit', {
  ...
  const handleSuggestionClick = (suggestion: SuggestedPrompt) => {
    trackEvent('[StartWithAISimple].selectSuggestion', { promptLength: suggestion.prompt.length });
