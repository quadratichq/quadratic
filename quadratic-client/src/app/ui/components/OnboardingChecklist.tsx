...
import { useAnimateElement } from '@/app/ui/hooks/useAnimateElement';
...
export const OnboardingChecklist = () => {
  ...
  const [visibility, setVisibility] = useState<'visible' | 'hidden'>(showOnboardingChecklist ? 'visible' : 'hidden');
  const onHide = useCallback(() => {
    setVisibility('hidden');
  }, []);

  const containerRef = useRef<HTMLDivElement | null>(null);
  const { transform, transition, animate } = useAnimateElement({
    ref: containerRef as RefObject<HTMLElement | null>,
    animateToId: 'onboarding-checklist-trigger',
  ...
  useEffect(() => {
    if (!initialRender.current) {
      animate(showOnboardingChecklist);
    } else {
  ...
  return (
    <div
      ref={containerRef}
      className="fixed bottom-[65px] right-2 rounded-lg border bg-background shadow-sm"
      style={{
        transform,
        transition,
        overflow: 'hidden',
