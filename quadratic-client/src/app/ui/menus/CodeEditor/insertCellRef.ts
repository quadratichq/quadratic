...
export const insertCellRef = (editorInteractionState: EditorInteractionState, relative?: boolean) => {
  ...
  if (language === 'Formula') {
    ...
    if (cursor.multiCursor) {
      ...
      const end = { x: multiCursor.right - 1, y: multiCursor.bottom - 1 };
      if (sheet) {
        ref = `let data = await cells(${start.x}, ${start.y}, ${end.x}, ${end.y}, '${sheet}')`;
      } else {
        if (relative) {
          ref = `let data = await relCells(${start.x - selectedCell.x}, ${start.y - selectedCell.y}, ${end.x - selectedCell.x}, ${
            end.y - selectedCell.y
          })`;
        } else {
          ref = `let data = await cells(${start.x}, ${start.y}, ${end.x}, ${end.y})`;
        }
      }
    } else {
      const location = cursor.cursorPosition;
      if (sheet) {
        ref = `let data = await cell(${location.x}, ${location.y}, '${sheet}')`;
      } else {
        if (relative) {
          ref = `let data = await relCell(${location.x - selectedCell.x}, ${location.y - selectedCell.y})`;
        } else {
          ref = `let data = await cell(${location.x}, ${location.y})`;
        }
      }
    }
  }
