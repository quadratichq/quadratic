...
export function useSubmitAIAnalystPrompt() {
  ...
  const updateInternalContext = useCallback(
  ...
  const submitPrompt = useCallback(
    async ({ messageSource, content, context, messageIndex, importFiles }: SubmitAIAnalystPromptArgs) => {
      ...
      const prevPromptSuggestions = aiStore.get(promptSuggestionsAtom);
      prevPromptSuggestions.abortController?.abort();
      ...
      const currentMessageCount = aiStore.get(currentChatMessagesCountAtom);
      if (messageIndex === 0) {
        aiStore.set(currentChatAtom, {
          id: v4(),
          name: '',
      ...
      // fork chat, if we are editing an existing chat
        ...
        const prevChat = aiStore.get(currentChatAtom);
        aiStore.set(currentChatAtom, {
          id: v4(),
          name: '',
          lastUpdated: Date.now(),
          messages: prevChat.messages.slice(0, messageIndex).map((message) => ({ ...message })),
        });
      }
      ...
      const onExceededBillingLimit = (exceededBillingLimit: boolean) => {
        ...
        const currentMessage = [...prevMessages];
        currentMessage.pop();
        messageIndex = currentMessage.length - 1;
        aiStore.set(currentChatMessagesAtom, currentMessage);

        aiStore.set(waitingOnMessageIndexAtom, messageIndex);
      ...
      abortController.signal.addEventListener('abort', () => {
        ...
        const prevPdfImport = aiStore.get(pdfImportAtom);
        prevPdfImport.abortController?.abort();
        aiStore.set(pdfImportAtom, { abortController: undefined, loading: false });

        const prevWebSearch = aiStore.get(webSearchAtom);
        prevWebSearch.abortController?.abort();
        ...
        if (!!lastMessage && isAIPromptMessage(lastMessage)) {
          ...
          aiStore.set(currentChatMessagesAtom, [...prevMessages.slice(0, -1), newLastMessage]);
          ...
          if (prevWaitingOnMessageIndex !== undefined) {
            ...
            aiStore.set(currentChatMessagesAtom, [...prevMessages, newLastMessage]);
          }
        }
      ...
      const prevMessagesForUser = aiStore.get(currentChatMessagesAtom);
      aiStore.set(currentChatMessagesAtom, [...prevMessagesForUser, { ...userMessage }]);
      ...
      const prevChat = aiStore.get(currentChatAtom);
      let chatId = prevChat.id ? prevChat.id : v4();
      let chatMessages: ChatMessage[] = [...prevChat.messages];
      aiStore.set(currentChatAtom, {
        ...prevChat,
        id: chatId,
        lastUpdated: Date.now(),
      ...
      try {
        ...
        while (toolCallIterations < MAX_TOOL_CALL_ITERATIONS) {
          ...
          aiStore.set(currentChatMessagesAtom, chatMessages);
          ...
          const response = await handleAIRequestToAPI({
            ...
            setMessages: (updater) => {
              ...
              aiStore.set(currentChatMessagesAtom, newMessages);
            },
            signal: abortController.signal,
            onExceededBillingLimit,
          });

          const waitingOnMessageIndexValue = aiStore.get(waitingOnMessageIndexAtom);
          if (waitingOnMessageIndexValue !== undefined) {
            break;
          }
          ...
          const prevMessagesForReplace = aiStore.get(currentChatMessagesAtom);
          let nextChatMessages: ChatMessage[] = replaceOldGetToolCallResults(prevMessagesForReplace);
          aiStore.set(currentChatMessagesAtom, nextChatMessages);
          ...
          for (const toolCall of webSearchToolCalls) {
            ...
            if (internal) {
              const prevMessagesForInternal = aiStore.get(currentChatMessagesAtom);
              nextChatMessages = [...prevMessagesForInternal, internal];
              aiStore.set(currentChatMessagesAtom, nextChatMessages);
              chatMessages = nextChatMessages;
            }
          }
          ...
          const prevMessagesForFiles = aiStore.get(currentChatMessagesAtom);
          nextChatMessages = [
            ...removeOldFilesInToolResult(prevMessagesForFiles, filesInToolResult),
            toolResultMessage,
          ];
          aiStore.set(currentChatMessagesAtom, nextChatMessages);
          chatMessages = nextChatMessages;
        ...
        const prevMessagesForError = aiStore.get(currentChatMessagesAtom);
        const lastMessage = prevMessagesForError.at(-1);
        if (!!lastMessage && isAIPromptMessage(lastMessage)) {
          const newLastMessage = { ...lastMessage };
          let currentContent = { ...(newLastMessage.content.at(-1) ?? createTextContent('')) };
          if (currentContent?.type !== 'text') {
          ...
          newLastMessage.toolCalls = [];
          newLastMessage.content = [...newLastMessage.content.slice(0, -1), currentContent];
          aiStore.set(currentChatMessagesAtom, [...prevMessagesForError.slice(0, -1), newLastMessage]);
        } else if (lastMessage?.role === 'user') {
          const newLastMessage: AIMessage = {
            role: 'assistant',
          ...
          aiStore.set(currentChatMessagesAtom, [...prevMessagesForError, newLastMessage]);
        }
