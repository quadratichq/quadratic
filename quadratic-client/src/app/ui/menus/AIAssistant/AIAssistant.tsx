...
export const AIAssistant = ({ autoFocus }: { autoFocus?: boolean }) => {
  ...
  const { mode, showCodeEditor } = useRecoilValue(editorInteractionStateAtom);
  const setShowAI = useSetRecoilState(showAIAtom);
  const { quadraticContext, aiContextReassertion } = useContextMessages();
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="start" alignOffset={-4}>
        {enabledModels.map((model) => {
          const displayName = MODEL_OPTIONS[model].displayName;
          return (
            <DropdownMenuCheckboxItem
              key={model}
