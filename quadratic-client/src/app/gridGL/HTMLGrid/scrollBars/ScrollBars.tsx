...
export const ScrollBars = (props: ScrollBarsProps) => {
  ...
  const pointerMoveHorizontal = useCallback(
    ...
    [down, lastViewport.scrollbarScaleX, start, state]
  );
  ...
  const pointerMoveVertical = useCallback(
    ...
    [state, down, start, lastViewport.scrollbarScaleY]
  );
  ...
  const calculateSize = useCallback(
    (
      ...
      if (!state && viewportSize >= contentSize && viewportStart <= -headingSize / pixiApp.viewport.scaled) {
        return undefined;
      }
      ...
      if (state === 'horizontal') {
        ...
        if (viewportBounds.right < contentSize.width) {
          ...
          newLastViewport.scrollbarScaleX =
            (viewportBounds.width / newLastViewport.horizontalBarWidth) * pixiApp.viewport.scaled;
        } else {
          // adjusts when we are past the end of the content
          newLastViewport.scrollbarScaleX =
            (viewportBounds.right / newLastViewport.scrollbarAreaWidth) * pixiApp.viewport.scaled;
        }
      }
      ...
      if (state === 'vertical') {
        ...
        if (viewportBounds.bottom < contentSize.height) {
          ...
          newLastViewport.scrollbarScaleY =
            (viewportBounds.height / newLastViewport.verticalBarHeight) * pixiApp.viewport.scaled;
        } else {
          // adjusts when we are past the end of the content
          newLastViewport.scrollbarScaleY =
            (viewportBounds.bottom / newLastViewport.scrollbarAreaHeight) * pixiApp.viewport.scaled;
        }
      }
  ...
  }, [calculateSize, lastViewport, props.sheetBounds, state]);
