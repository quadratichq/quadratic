...
export class GridHeadings extends Container {
  ...
  private calculateCharacterSize() {
    const label = new BitmapText({ text: 'X', style: { fontFamily: 'OpenSans', fontSize: GRID_HEADER_FONT_SIZE } });
  ...
  private drawHorizontalBar() {
    ...
    this.headingsGraphics.fill(colors.headerBackgroundColor);
  ...
  private horizontalLabels() {
    ...
    for (let x = leftOffset; x <= rightOffset; x += currentWidth) {
      ...
      if (gridAlpha !== 0) {
        this.headingsGraphics.strokeStyle = {
          width: 1,
  ...
  private drawCorner() {
    ...
    this.corner.fill(colors.headerCornerBackgroundColor);
    this.corner.strokeStyle = {
      width: 1,
      color: colors.gridLines,
      alpha: colors.headerSelectedRowColumnBackgroundColorAlpha,
      alignment: 0.5,
