...
type UndoResponse = z.infer<(typeof aiToolsSpec)[AITool.Undo]['responseSchema']>;
  ...
  ({ toolCall: { arguments: args, loading }, className }: { toolCall: AIToolCall; className: string }) => {
    ...
    useEffect(() => {
      ...
      try {
        ...
        setToolArgs(aiToolsSpec[AITool.Undo].responseSchema.safeParse(json));
      } catch (error) {
        setToolArgs(undefined);
...
export const Redo = memo(
  ({ toolCall: { arguments: args, loading }, className }: { toolCall: AIToolCall; className: string }) => {
    const [toolArgs, setToolArgs] = useState<z.SafeParseReturnType<RedoResponse, RedoResponse>>();
