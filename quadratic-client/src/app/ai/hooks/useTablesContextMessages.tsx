...
export function useTablesContextMessages() {
  const getTablesContext = useCallback(async (): Promise<ChatMessage[]> => {
    ...
    const allCodeTables = tablesContext.flatMap((tx) => tx.code_tables);
    const filteredFormulaTables = allCodeTables.filter((table) => table.language === 'Formula');
    const formulaTables = filteredFormulaTables.map((table) => {
      const baseTable = {
    ...
    const codeTables = allCodeTables.filter((table) => table.language !== 'Formula');
...
- Code tables: These are read only tables that are generated by running code in code cells (Python, Javascript, etc.).\n
- Formula tables: These are read only tables that are generated by running formulas in formula cells.\n
...
- first_row_visible_values: This is an array of visible values in the first row of the code table.\n
- last_row_visible_values: This is an array of visible values in the last row of the code table.\n
- bounds: This is the bounds (top left cell and bottom right cell, both inclusive) of the code table in A1 notation, this includes the table name and column headers if they are visible.\n
- intended_bounds: This is the desired bounds for the table (which may be different from the bounds if the table has a spill).\n
- show_name: This is a boolean value representing if the table name is visible.\n
- show_columns: This is a boolean value representing if the column headers are visible.\n
- language: This is the language of the code cell that generates this table (Python, Javascript, etc.).\n
...
${
  formulaTables.length > 0
    ? `
I am sharing formula tables in the currently open file for reference as an array of objects described below:\n
Each formula table has following properties:\n
- sheet_name: This is the name of the sheet.\n
- bounds: This is the bounds (top left cell and bottom right cell, both inclusive) of the formula table in A1 notation, this includes the table name and column headers if they are visible.\n
- spill: This is a boolean value representing if the formula table has a spill.\n
${
  formulaTables.length <= 500
    ? `- first_row_visible_values: This is an array of visible values in the first row of the formula table.\n- error: This is a boolean value representing if the formula table has an error.\n`
    : `Note: Due to large number of formula tables (${formulaTables.length}), first_row_visible_values and error properties are omitted to reduce context size.\n`
}\n

Note: Formula table details (formula content, error details) are provided separately in code cell context when working with specific formula cells.\n\n
...
\n\n
`
    : 'There are no formula tables in the currently open file.\n\n'
}
...
- In formulas, cell references are done using A1 notation directly, without quotes. Example: \`=SUM(Table_Name[#DATA])\` or \`=SUM(Table_Name[#HEADERS])\` or \`=SUM(Table_Name[#ALL])\` or \`=SUM(Table_Name[Column_Name])\`.\n
...
- Use table names (Table_Name) when working with entire tables. Use A1 notation for non-table data or partial table selections.\n
...
- In Formulas and JavaScript use \`q.cells("Table_Name[#DATA]")\` to reference the data of the table. This does not work in Python.\n`,
      ...
      },
      {
        ...
        content: [
          {
            type: 'text',
            text: `I understand the table and chart information, I will reference it to answer following messages. How can I help you?`,
          },
        ],
        contextType: 'tables',
      },
