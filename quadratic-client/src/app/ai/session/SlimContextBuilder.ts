...
import { getConnectionMarkdown, getConnectionTableInfo } from '../utils/aiConnectionContext';
 ...
 */
export class SlimContextBuilder {
  /**
   * Build slim context for an AI request.
  ...
  async buildContext(options: ContextOptions): Promise<ChatMessage[]> {
    const results = await Promise.allSettled([
      ...
      this.getSlimSummaryContext(), // SLIM: No cell values
    ...
    const [sqlContext, filesContext, visibleContext, summaryContext, codeErrorContext, aiTransactions] = results.map(
      (result, index) => {
        ...
        console.warn(`[SlimContextBuilder] Failed to get ${contextNames[index]} context:`, result.reason);
        return [];
      }
    );
        ...
        connections
          ...
          .map(async (connection) => {
            try {
              ...
              console.warn(`[SlimContextBuilder] Failed to get table names for connection ${connection.uuid}:`, error);
              return '';
            }
          })
      );
...
${contextText}`),
      ...
      console.error('[SlimContextBuilder] Error fetching SQL context:', error);
      return [];
    }
  }
...
How can I help you?`
  ...
  /**
   * Get visible area context (slim version - no cell values)
   */
...
`;
   ...
   * This is the key difference from ContextBuilder.
   */
  async getSlimSummaryContext(): Promise<ChatMessage[]> {
    if (!fileHasData()) {
      return [
        {
          role: 'user',
    ...
    const sheetsContext = await quadraticCore.getAISelectionContexts({
      selections,
      maxRows: 0, // Don't fetch any row data
    });
...
## '${sheetContext.sheet_name}' summary\n\n`;
      ...
      // Data Tables - BOUNDS ONLY, no cell values
      if (sheetContext.data_tables && sheetContext.data_tables.length > 0) {
      ...
      // Code Tables - BOUNDS ONLY, no cell values
      if (sheetContext.code_tables && sheetContext.code_tables.length > 0) {
      ...
      // Connection Tables - BOUNDS ONLY, no cell values
      if (sheetContext.connections && sheetContext.connections.length > 0) {
        text += `\n### '${sheet.name}' Connection tables:\n`;
        for (const table of sheetContext.connections) {
          if (typeof table.language !== 'object' || !table.language.Connection) {
            console.warn('Unexpected non-connection table in getSlimSummaryContext');
            continue;
          }
      ...
      // Flat Data - RANGES ONLY, no cell values
      if (sheetContext.data_rects && sheetContext.data_rects.length > 0) {
    ...
    // Add instruction about using subagent for data exploration
    text += `
