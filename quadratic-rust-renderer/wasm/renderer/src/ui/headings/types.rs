...
impl Default for HeadingColors {
    fn default() -> Self {
        Self {
            background: [1.0, 1.0, 1.0, 1.0], // White (0xffffff - headerBackgroundColor in colors.ts)
            corner_background: [1.0, 1.0, 1.0, 1.0], // White (same as background)
            label: [0.137, 0.192, 0.263, 1.0], // #233143 (gridHeadingLabel in colors.ts)
            grid_line: [0.8, 0.8, 0.8, 1.0],   // Grid line gray
            selection: [0.141, 0.388, 0.922, 1.0], // #2463eb (headerSelectedBackgroundColor)
            selection_alpha: 0.1, // headerSelectedBackgroundColorAlpha
        }
    }
}
