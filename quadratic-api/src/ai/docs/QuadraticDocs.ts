...
In A1 notation, when referencing an entire column or row, use the column or row name(s). For example, if the user wants to do something on Column B, use "B", and not "B1:B100". Similarly, if the user wants to do something on Row 10, use "10", and not "A10:Z10". You can also start from a specific row or column, for example "C4:C" will capture all rows in column C starting from row 4, or "D2:2" will capture all columns in row 2 starting from column D.
