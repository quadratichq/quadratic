...
import { assertValidationMessage } from './helpers/sheet.helper';
...
test('Validations', async ({ page }) => {
  ...
  // Range (max only)
  await assertValidationMessage(page, 'B23', 'Pick a number', 'Choose number < 99');
  await assertValidationMessage(page, 'C23', 'Pick a number', 'Choose number < 99');
  await assertValidationMessage(page, 'D23', 'Invalid Number', 'Choose number < 99');
  ...
  // DT Equals (no time)
  await assertValidationMessage(page, 'B27', 'Date equals', '01/01/2001');
  await assertValidationMessage(page, 'C27', 'Validation Warning', '01/01/2001');
  await assertValidationMessage(page, 'D27', 'Validation Warning', '01/01/2001');

  // DT Not Equals
  await assertValidationMessage(page, 'B27', 'Date not equal', '01/01/2001');
  await assertValidationMessage(page, 'C27', 'Date not equal', '01/01/2001');
  await assertValidationMessage(page, 'D27', 'Validation Warning', '01/01/2001');

  // DT Range
  await assertValidationMessage(page, 'B28', 'DT range', '01/01/2001 and 01/03/2003');
  await assertValidationMessage(page, 'C28', 'Validation Warning', '01/01/2001 and 01/03/2002');
