...
test.skip('AI Prompt - Create Chart using AI Prompt', async ({ page, context }) => {
  ...
  await page.getByRole(`menuitem`, { name: `Move to origin` }).click();
...
test.skip('AI Sheet Data analysis', async ({ page }) => {
  ...
  await page.locator('span.material-symbols-outlined:has-text("add")').click();
  ...
  await page.locator('div[data-title="Sheet 1"]').click();

  // Click the "add" icon to add the new sheet to the chat
  await page.locator('span.material-symbols-outlined:has-text("add") >>nth=1').click();

  // Click the "Sheet2" menu item to add it to the chat context
  await page.locator('div[role="menuitemcheckbox"]:has-text("Sheet 2")').click();
...
test.skip('Think Enabled Response', async ({ page }) => {
  ...
  await page.getByText(`Show thinking`).first().click();
