...
export function createMcpServer(bridge) {
    ...
    function relay(name, description, schema, handler) {
    ...
    // Cell Data
    // ---------------------------------------------------------------------------
    relay("set_cell_values", "Set values for a rectangular block of cells. Provide the top-left cell " +
    ...
    relay("get_cell_data", "Get the values of cells in a selection. Returns cell data as markdown.", {
    ...
    relay("has_cell_data", "Check if any cells in the chosen selection have data. Returns true if any cell contains data.", {
        sheet_name: STR_OPT.describe("Sheet name"),
        selection: STR.describe('Selection in A1 notation, e.g. "A1:B10"'),
    });
    relay("add_data_table", "Add a data table to a sheet. The first row of data is treated as the header row.", {
    ...
    relay("move_cells", "Move a rectangular selection of cells to a new location.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    relay("delete_cells", "Delete the values of a selection of cells.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    // Code
    // ---------------------------------------------------------------------------
    relay("set_code_cell_value", "Set a code cell that will be evaluated by Quadratic. " +
    ...
    relay("get_code_cell_value", "Get the full code for an existing Python, JavaScript, Formula, or connection cell.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    relay("set_formula_cell_value", "Set one or more formula cells. Each formula has a sheet, position, and formula string. " +
        "Do NOT prefix formulas with =.", {
        formulas: z
            .array(z.object({
            sheet_name: STR_OPT.describe("Sheet name"),
            code_cell_position: STR.describe('Cell position or range in A1 notation, e.g. "A1" or "A1:A10"'),
            formula_string: STR.describe("The formula to run (without = prefix)"),
        }))
            .min(1)
            .describe("Array of formulas to set"),
    });
    relay("rerun_code", "Re-run code cells. Optionally scope to a sheet or selection.", {
    ...
    // Connections
    // ---------------------------------------------------------------------------
    relay("get_database_schemas", "Get the schemas for one or more database connections.", {
        connection_ids: z
            .array(STR)
            .describe("Array of connection UUIDs to get schemas for"),
    });
    relay("set_sql_code_cell_value", "Set a SQL code cell that queries a database connection.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    // Format
    // ---------------------------------------------------------------------------
    relay("set_text_formats", "Set text formatting (bold, italic, colors, alignment, number format, etc.) on one or more selections.", {
        formats: z
            .array(z.object({
            sheet_name: STR_OPT.describe("Sheet name"),
    ...
    relay("get_text_formats", "Get the text formatting of a selection of cells.", {
        sheet_name: STR_OPT.describe("Sheet name"),
        selection: STR.describe("Selection in A1 notation"),
        page: NUM.describe("Page number (0-based)"),
    });
    relay("set_borders", "Set borders on a selection of cells.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    // Tables
    // ---------------------------------------------------------------------------
    relay("convert_to_table", "Convert a selection of cells into a data table.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    relay("table_meta", "Update metadata for a table (name, display options).", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    relay("table_column_settings", "Rename, show, or hide columns in a table.", {
        sheet_name: STR_OPT.describe("Sheet name"),
    ...
    relay("set_default_column_width", "Set the default column width for the entire sheet.", {
        sheet_name: STR_OPT.describe("Sheet name"),
        size: z
            .number()
            .min(20)
    ...
    relay("set_default_row_height", "Set the default row height for the entire sheet.", {
        sheet_name: STR_OPT.describe("Sheet name"),
        size: z
            .number()
            .min(10)
    ...
    relay("add_logical_validation", "Add a logical (checkbox) validation to a selection.", {
        sheet_name: STR_OPT.describe("Sheet name"),
        selection: STR.describe("Selection in A1 notation"),
        show_checkbox: BOOL_OPT.describe("Show checkbox UI"),
        ignore_blank: BOOL_OPT.describe("Allow blank values"),
    });
    relay("add_list_validation", "Add a dropdown list validation to a selection.", {
        sheet_name: STR_OPT.describe("Sheet name"),
        selection: STR.describe("Selection in A1 notation"),
        ignore_blank: BOOL_OPT.describe("Allow blank values"),
    ...
    // ---------------------------------------------------------------------------
    relay("get_conditional_formats", "Get all conditional formatting rules for a sheet.", {
        sheet_name: STR.describe("Sheet name"),
    });
    relay("update_conditional_formats", "Create, update, or delete conditional formatting rules.", {
        sheet_name: STR.describe("Sheet name"),
        rules: z
            .array(z.object({
    ...
    // Misc
    // ---------------------------------------------------------------------------
    relay("text_search", "Search for text in cells within a sheet or the entire file.", {
    ...
    // ---------------------------------------------------------------------------
    relay("get_sheet_info", "List all sheets and their names in the spreadsheet.", {});
    return mcp;
}
