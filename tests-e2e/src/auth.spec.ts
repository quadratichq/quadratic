...
test('Log Out', async ({ page }) => {
  ...
  await expect(page.locator(`:text("Login to Quadratic.")`)).toBeVisible({ timeout: 30 * 1000 });
  await expect(page.locator(`#username`)).toBeVisible({ timeout: 30 * 1000 });
  await expect(page.locator(`#password`)).toBeVisible({ timeout: 30 * 1000 });
