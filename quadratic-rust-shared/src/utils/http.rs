...
/// Build a shared HTTP client with standard timeout settings.
...
/// `MAX_DOWNLOAD_SIZE` based on both the Content-Length header and
/// actual downloaded size.
pub async fn download_file(client: &reqwest::Client, url: &str) -> Result<bytes::Bytes> {
    let response = client
        .get(url)
        .send()
        .await
