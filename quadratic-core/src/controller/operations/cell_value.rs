...
impl GridController {
    ...
    pub(super) fn string_to_cell_value(
        ...
        } else if let Some(time) = CellValue::parse_time(value) {
            time
        } else if let Some(date) = CellValue::parse_date(value) {
            date
        } else if let Some(date_time) = CellValue::parse_date_time(value) {
            date_time
        } else if let Some(duration) = CellValue::parse_duration(value) {
            duration
