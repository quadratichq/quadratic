...
impl GridController {
    ...
    pub fn thumbnail_dirty_pos(&self, sheet_pos: SheetPos) -> bool {
        self.thumbnail_dirty_rect(SheetRect::single_pos(sheet_pos))
    }

    /// whether the thumbnail needs to be updated for this rectangle
    pub fn thumbnail_dirty_rect(&self, sheet_rect: SheetRect) -> bool {
...
mod test {
    ...
    fn test_thumbnail_dirty_pos() {
        ...
        assert!(gc.thumbnail_dirty_pos(SheetPos {
            x: 0,
            y: 0,
            sheet_id
        }));
    ...
    fn test_thumbnail_dirty_rect() {
        let gc = GridController::new();
        let sheet_id = gc.sheet_ids()[0];
        assert!(gc.thumbnail_dirty_rect(SheetRect {
            min: Pos { x: 0, y: 0 },
            max: Pos { x: 1, y: 1 },
            sheet_id,
        }));
        ...
        assert!(!gc.thumbnail_dirty_rect(SheetRect::single_pos(SheetPos {
            x: THUMBNAIL_WIDTH as i64,
            y: THUMBNAIL_HEIGHT as i64,
            sheet_id,
        }),));
