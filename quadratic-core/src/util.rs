...
pub fn column_name(mut n: i64) -> String {
    ...
    while n != 0 {
        n -= signum;
        let i = (n % 26).abs();
        ret.push(b"ABCDEFGHIJKLMNOPQRSTUVWXYZ"[i as usize] as char);
        n /= 26;
    }
    if signum <= 0 {
        ret.push('n');
...
pub fn column_from_name(mut s: &str) -> Option<i64> {
    if s == "n" {
        ...
        let signum = if let Some(rest) = s.strip_prefix('n') {
            s = rest;
            -1
        } else {
            1
...
mod tests {
    ...
    fn test_column_names() {
        ...
        assert_eq!("Z", column_name(26));
        assert_eq!("AA", column_name(27));
        assert_eq!("AB", column_name(28));
        ...
        // Test near ±52
        assert_eq!("AY", column_name(51));
        assert_eq!("AZ", column_name(52));
        assert_eq!("BA", column_name(53));
        assert_eq!("BB", column_name(54));
        ...
        // Test near ±702
        assert_eq!("ZY", column_name(701));
        assert_eq!("ZZ", column_name(702));
        assert_eq!("AAA", column_name(703));
        assert_eq!("AAB", column_name(704));
        ...
        assert_eq!("CRPXNLSKVLJFHG", column_name(i64::MAX));
        assert_eq!("nCRPXNLSKVLJFHH", column_name(i64::MIN));

        // Test fun stuff
        assert_eq!("QUADRATIC", column_name(3719092809669));
        assert_eq!("nQUADRATIC", column_name(-3719092809669));
        assert_eq!("QUICKBROWNFOX", column_name(1700658608758053878));
    }
    ...
    fn test_from_column_names() {
        ...
        assert_eq!(Some(0), column_from_name("n"));
        ...
        assert_eq!(Some(26), column_from_name("Z"));
        assert_eq!(Some(27), column_from_name("AA"));
        assert_eq!(Some(28), column_from_name("AB"));
        ...
        // Test near ±52
        assert_eq!(Some(51), column_from_name("AY"));
        assert_eq!(Some(52), column_from_name("AZ"));
        assert_eq!(Some(53), column_from_name("BA"));
        assert_eq!(Some(54), column_from_name("BB"));
        ...
        // Test near 702
        assert_eq!(Some(701), column_from_name("ZY"));
        assert_eq!(Some(702), column_from_name("ZZ"));
        assert_eq!(Some(703), column_from_name("AAA"));
        assert_eq!(Some(704), column_from_name("AAB"));
        ...
        // Test near the integer limits
        assert_eq!(Some(i64::MAX), column_from_name("CRPXNLSKVLJFHG"));
        assert_eq!(Some(i64::MIN), column_from_name("nCRPXNLSKVLJFHH"));
        assert_eq!(None, column_from_name("CRPXNLSKVLJFHH"));
        ...
        // Test fun stuff
        assert_eq!(Some(3719092809669), column_from_name("QUADRATIC"));
        assert_eq!(Some(1700658608758053878), column_from_name("QUICKBROWNFOX"));
    }
}
