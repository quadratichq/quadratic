...
impl Sheet {
    ...
    /// Finds the rectangular area that contains data from an infinite range.
    pub fn find_rect_from_infinite_range(&self, range: &CellRefRange) -> Option<Rect> {
        let mut start_col = range.start.col.map(|col| col.coord);
        let mut start_row = range.start.row.map(|row| row.coord);
        let mut end_col = range.end.and_then(|end| end.col).map(|col| col.coord);
        let mut end_row = range.end.and_then(|end| end.row).map(|row| row.coord);

        // swap so start is the one that is always defined.
        ...
        // handle start_col and start_row is defined
        if let (Some(start_col), Some(start_row)) = (start_col, start_row) {
            // handle start_col, start_row, end_col and end_row is defined
            if end_col.is_some() && end_row.is_some() {
                // this should not happen since it would be handled in the finite ranges above
            }
            // handle start_col, start_row, end_col is defined
            else if let Some(end_col) = end_col {
                let Some((_, calculated_end_row)) =
                    self.columns_bounds(start_col as i64, end_col as i64, true)
                else {
                    return None;
                };
                end_row = Some(calculated_end_row as u64);
            }
            // handle start_col, start_row, end_row is defined
            else if let Some(end_row) = end_row {
                let Some((_, calculated_end_col)) =
                    self.rows_bounds(start_row as i64, end_row as i64, true)
                else {
                    return None;
                };
                end_col = Some(calculated_end_col as u64);
            }
        }
        // handle only start_col is defined
        else if let Some(start_col) = start_col {
            // handle start_col, end_col is defined
            if let Some(end_col) = end_col {
                let Some((calculated_start_row, calculated_end_row)) =
                    self.columns_bounds(start_col as i64, end_col as i64, true)
                else {
                    return None;
                };
                start_row = Some(calculated_start_row as u64);
                end_row = Some(calculated_end_row as u64);
            } else {
                // handle only start_col
                let Some((calculated_start_row, calculated_end_row)) =
                    self.column_bounds(start_col as i64, true)
                else {
                    return None;
                };
                start_row = Some(calculated_start_row as u64);
                end_row = Some(calculated_end_row as u64);
                end_col = Some(start_col);
            }
        }
        // handle only start_row is defined
        else if let Some(start_row) = start_row {
            // handle start_row, end_row is defined
            if let Some(end_row) = end_row {
                let Some((calculated_start_col, calculated_end_col)) =
                    self.rows_bounds(start_row as i64, end_row as i64, true)
                else {
                    return None;
                };
                start_col = Some(calculated_start_col as u64);
                end_col = Some(calculated_end_col as u64);
            } else {
                // handle only start_row
                let Some((calculated_start_col, calculated_end_col)) =
                    self.row_bounds(start_row as i64, true)
                else {
                    return None;
                };
                start_col = Some(calculated_start_col as u64);
                end_col = Some(calculated_end_col as u64);
                end_row = Some(start_row);
            }
        }
        // handle completely infinite range
        else {
            let bounds = self.bounds(true);
            match bounds {
                GridBounds::Empty => return None,
                GridBounds::NonEmpty(rect) => {
                    start_col = Some(rect.min.x as u64);
                    start_row = Some(rect.min.y as u64);
                    end_col = Some(rect.max.x as u64);
                    end_row = Some(rect.max.y as u64);
                }
            }
        }

        // If all boundaries are defined, then we can create the rect
