...
impl Sheet {
    /// Returns a (col_size, row_size) for the values based on the number of
    /// columns and rows. Note: this is not optimized for sparse content.
    fn find_col_row_block_size(cols: usize, rows: usize) -> (usize, usize) {
        let max = MAX_OPERATION_SIZE_COL_ROW as usize;
        let total_cells = cols * rows;
        if total_cells > max {
            // Since we store data in columns, optimize block size to minimize column splits
            let num_rows = if rows < max { rows } else { max };
            let num_cols = ((max / num_rows) as usize).max(1);
    ...
    fn reverse_values_ops_for_columns(&self, cols: Vec<i64>) -> Vec<Operation> {
        let mut reverse_operations = Vec::new();

        if let Some((min, max)) = self.columns_bounds(cols[0], cols[cols.len() - 1], true) {
            let (num_cols, num_rows) = Self::find_col_row_block_size(cols.len(), (max - min) as usize);

            let mut x= cols[0];
            let mut y = min;
            let mut count = 0;
            while count < (max - min) as usize * cols.len() {

            }
            for col in cols {
                let size = if max - min > MAX_OPERATION_SIZE_COL_ROW {
    ...
    fn reverse_formats_ops_for_columns(&self, column: i64) -> Vec<Operation> {
    ...
    fn reverse_borders_ops_for_columns(&self, column: i64) -> Vec<Operation> {
    ...
    fn reverse_code_runs_ops_for_columns(&self, column: i64) -> Vec<Operation> {
        let mut reverse_operations = Vec::new();
    ...
    fn delete_column_offsets(&mut self, transaction: &mut PendingTransaction, column: i64) {
    ...
    pub(crate) fn delete_columns(&mut self, transaction: &mut PendingTransaction, column: i64) {
        // create undo operations for the deleted column (only when needed since
        // it's a bit expensive)
        if transaction.is_user_undo_redo() {
            transaction
                .reverse_operations
                .extend(self.reverse_borders_ops_for_columns(column));
            transaction
                .reverse_operations
                .extend(self.reverse_formats_ops_for_columns(column));
            transaction
                .reverse_operations
                .extend(self.reverse_code_runs_ops_for_columns(column));
            // transaction
            //     .reverse_operations
            //     .extend(self.reverse_values_ops_for_columns(column));
        }

        self.delete_column_offsets(transaction, column);
...
mod tests {
    ...
    #[test]
    fn test_find_col_row_block_size() {
        // max is 1000 cells

        // 10x10 cells
        let (num_cols, num_rows) = Sheet::find_col_row_block_size(10, 10);
        assert_eq!(num_cols, 10);
        assert_eq!(num_rows, 10);

        // 10x1000
        let (num_cols, num_rows) = Sheet::find_col_row_block_size(10, 1000);
        assert_eq!(num_cols, 1);
        assert_eq!(num_rows, 1000);

        // 10x1001
        let (num_cols, num_rows) = Sheet::find_col_row_block_size(10, 1001);
        assert_eq!(num_cols, 1);
        assert_eq!(num_rows, 1000);

        // 1001x10
        let (num_cols, num_rows) = Sheet::find_col_row_block_size(1001, 10);
        assert_eq!(num_cols, 100);
        assert_eq!(num_rows, 10);
    }
    ...
    //     let ops = sheet.reverse_values_ops_for_columns(2);
    //     assert_eq!(ops.len(), 1);
    // }
