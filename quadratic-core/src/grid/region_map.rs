...
pub struct RegionMap {
    ...
    region_to_pos: HashMap<SheetId, RTree<GeomWithData<Rect, MultiPos>>>,
    ...
    pos_to_region: HashMap<SheetId, HashMap<MultiPos, Vec<(SheetId, Rect)>>>,
}
impl RegionMap {
    ...
    pub fn insert(&mut self, pos: MultiPos, region: (SheetId, Rect)) {
    ...
    /// unbounded.
    pub fn set_regions_for_pos(&mut self, pos: MultiPos, regions: Vec<(SheetId, Rect)>) {
        self.remove_pos(pos);
    ...
    pub fn remove_sheet(&mut self, sheet_id: SheetId) {
        ...
        let positions: HashSet<MultiPos> = rtree
            .into_iter()
            .map(|obj| obj.data)
    ...
    pub fn remove_pos(&mut self, pos: MultiPos) {
    ...
    #[cfg(test)]
    fn find_all_mentions_of_sheet(&self, sheet_id: SheetId) -> Vec<(MultiPos, (SheetId, Rect))> {
        itertools::chain(
...
impl Serialize for RegionMap {
    ...
    where
        S: serde::Serializer,
    {
        let associations: Vec<(MultiPos, (SheetId, Rect))> = self
            .pos_to_region
            .iter()
            .flat_map(|(_, map)| {
                map.iter()
...
impl<'de> Deserialize<'de> for RegionMap {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        for (pos, region) in Vec::<(MultiPos, (SheetId, Rect))>::deserialize(deserializer)? {
...
impl PartialEq for RegionMap {
    fn eq(&self, other: &Self) -> bool {
        let region_to_pos_self: HashSet<(MultiPos, (SheetId, Rect))> = self
            .region_to_pos
            .iter()
        ...
        let region_to_pos_other: HashSet<(MultiPos, (SheetId, Rect))> = other
            .region_to_pos
            .iter()
            .flat_map(|(sheet_id, rtree)| {
                rtree.iter().map(|obj| (obj.data, (*sheet_id, *obj.geom())))
        ...
        let pos_to_region_other: HashSet<(MultiPos, (SheetId, Rect))> = other
            .pos_to_region
            .iter()
            .flat_map(|(_, map)| {
                map.iter()
