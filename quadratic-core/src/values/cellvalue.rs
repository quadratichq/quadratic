...
impl CellValue {
    ...
    pub fn unpack_percentage(s: &str) -> Option<Decimal> {
        ...
        if without_parentheses.ends_with("%") {
            ...
            if let Ok(decimal) = Decimal::from_str(&without_commas) {
    ...
    pub fn unpack_currency(s: &str) -> Option<(String, Decimal)> {
        ...
        for char in CURRENCY_SYMBOLS.chars() {
                ...
                .map(|stripped| stripped.trim())
            {
                ...
                if let Ok(bd) = Decimal::from_str(&without_commas) {
    ...
    pub fn unpack_str_float(value: &str, default: CellValue) -> CellValue {
        Decimal::from_str(value).map_or_else(|_| default, CellValue::Number)
    }
    ...
    pub fn parse_from_str(value: &str) -> CellValue {
        ...
        let number = Decimal::from_str(&without_commas);
    ...
    pub fn string_to_cell_value(value: &str, allow_code: bool) -> (CellValue, FormatUpdate) {
        ...
        let cell_value = if value.is_empty() {
            ...
            bool
        } else if let Ok(bd) = Decimal::from_str(&CellValue::strip_commas(
...
mod test {
    ...
    fn test_cell_value_to_display_number() {
        let cv = CellValue::Number(Decimal::from_str("123123.1233").unwrap());
        assert_eq!(cv.to_display(), String::from("123123.1233"));

        let cv = CellValue::Number(Decimal::from_str("-123123.1233").unwrap());
        assert_eq!(cv.to_display(), String::from("-123123.1233"));

        let cv = CellValue::Number(Decimal::from_str("123.1255").unwrap());
        assert_eq!(cv.to_display(), String::from("123.1255"));

        let cv = CellValue::Number(Decimal::from_str("123.0").unwrap());
    ...
    fn test_cell_value_to_display_percentage() {
        let cv = CellValue::Number(Decimal::from_str("0.015").unwrap());
        assert_eq!(
            cv.to_number_display(
                Some(NumericFormat {
                    kind: NumericFormatKind::Percentage,
        ...
        let cv = CellValue::Number(Decimal::from_str("0.9912239").unwrap());
        assert_eq!(
            cv.to_number_display(
                Some(NumericFormat {
                    kind: NumericFormatKind::Percentage,
        ...
        let cv = CellValue::Number(Decimal::from_str("1231123123.9912239").unwrap());
        assert_eq!(
            cv.to_number_display(
                Some(NumericFormat {
                    kind: NumericFormatKind::Percentage,
    ...
    fn to_number_display_scientific() {
        let cv = CellValue::Number(Decimal::from_str("12345678").unwrap());
        assert_eq!(
            cv.to_number_display(
                Some(NumericFormat {
        ...
        let cv = CellValue::Number(Decimal::from_str("1000000").unwrap());
        assert_eq!(
            cv.to_number_display(
                Some(NumericFormat {
                    kind: NumericFormatKind::Exponential,
    ...
    fn test_unpack_percentage() {
        ...
        assert_eq!(
            ...
            Some(Decimal::from_str("12.3812232").unwrap()),
        );
    }
    ...
    fn test_exponential_display() {
        let value = CellValue::Number(Decimal::from_str("98172937192739718923.12312").unwrap());
    ...
    #[test]
    fn to_get_cells() {
        let value = CellValue::Number(Decimal::from_str("123123.1233").unwrap());
    ...
    fn test_strip_negative_percent() {
        ...
        assert_eq!(
            value,
            CellValue::Number(Decimal::from_str("-123.123").unwrap())
        );
