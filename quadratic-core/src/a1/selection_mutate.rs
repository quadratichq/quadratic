...
mod tests {
    ...
    fn test_removed_column() {
        ...
        assert_eq!(selection, A1Selection::test("A1:A2"));
        ...
        assert_eq!(selection, A1Selection::test("A1:B3"));
        ...
        assert_eq!(selection, A1Selection::test("A1:C3"));

        // Multiple ranges with column removal
        let mut selection = A1Selection::test("A1:C3,E1:G3");
        assert!(selection.removed_column(2));
        assert_eq!(selection, A1Selection::test("A1:B3,D1:F3"));

        // Remove column that causes range deletion
        let mut selection = A1Selection::test("B1,C1:D1");
        assert!(selection.removed_column(2));
        assert_eq!(selection, A1Selection::test("B1:C1"));

        // Cursor adjustment when column is removed
        let mut selection = A1Selection::test("D1");
        ...
        let mut selection = A1Selection::test("A1:B3");
        selection.cursor.x = 4; // D column
        assert!(!selection.removed_column(3));
        assert_eq!(selection.cursor.x, 1);
        assert_eq!(selection, A1Selection::test("A1:B3"));
    ...
    fn test_removed_row() {
        ...
        assert_eq!(selection, A1Selection::test("A1:B1"));
        ...
        let mut selection = A1Selection::test("A1:B3");
        assert!(selection.removed_row(2));
        assert_eq!(selection, A1Selection::test("A1:B2"));
        ...
        let mut selection = A1Selection::test("A1:B3,A5:B7");
        assert!(selection.removed_row(2));
        assert_eq!(selection, A1Selection::test("A1:B2,A4:B6"));

        // Remove row that causes range deletion
        let mut selection = A1Selection::test("A2,A3:A4");
        assert!(selection.removed_row(2));
        assert_eq!(selection, A1Selection::test("A2:A3"));

        // Cursor adjustment when row is removed
        let mut selection = A1Selection::test("A4");
    ...
    fn test_inserted_column() {
        ...
        assert_eq!(selection, A1Selection::test("B1:C2"));

        // Insert in the middle of a range
        let mut selection = A1Selection::test("A1:C3");
        assert!(selection.inserted_column(2));
        assert_eq!(selection, A1Selection::test("A1:D3"));
        ...
        let mut selection = A1Selection::test("A1:B2,D1:E2");
        assert!(selection.inserted_column(3));
        assert_eq!(selection, A1Selection::test("A1:B2,E1:F2"));
        ...
        let mut selection = A1Selection::test("C1");
        assert!(selection.inserted_column(2));
        assert_eq!(selection, A1Selection::test("D1"));

        // Insert affecting multiple single-cell selections
        let mut selection = A1Selection::test("B1,D1,F1");
        assert!(selection.inserted_column(3));
        assert_eq!(selection, A1Selection::test("B1,E1,G1"));

        // Insert at column 1 (shifting everything right)
        let mut selection = A1Selection::test("A1:C3");
        assert!(selection.inserted_column(1));
        assert_eq!(selection, A1Selection::test("B1:D3"));

        // Insert in large range
        let mut selection = A1Selection::test("A1:Z3");
        assert!(selection.inserted_column(13));
        assert_eq!(selection, A1Selection::test("A1:AA3"));
    ...
    fn test_inserted_row() {
        ...
        assert_eq!(selection, A1Selection::test("A2:B3"));

        // Insert in the middle of a range
        let mut selection = A1Selection::test("A1:B3");
        assert!(selection.inserted_row(2));
        assert_eq!(selection, A1Selection::test("A1:B4"));
        ...
        let mut selection = A1Selection::test("A1:B2,A4:B5");
        assert!(selection.inserted_row(3));
        assert_eq!(selection, A1Selection::test("A1:B2,A5:B6"));
        ...
        // Insert affecting single-cell selection
        let mut selection = A1Selection::test("A3");
        assert!(selection.inserted_row(2));
        assert_eq!(selection, A1Selection::test("A4"));

        // Insert affecting multiple single-cell selections
        let mut selection = A1Selection::test("A2,A4,A6");
        assert!(selection.inserted_row(3));
        assert_eq!(selection, A1Selection::test("A2,A5,A7"));

        // Insert at row 1 (shifting everything down)
        let mut selection = A1Selection::test("A1:C3");
        assert!(selection.inserted_row(1));
        assert_eq!(selection, A1Selection::test("A2:C4"));

        // Insert in large range
        let mut selection = A1Selection::test("A1:C20");
        assert!(selection.inserted_row(13));
        assert_eq!(selection, A1Selection::test("A1:C21"));
    ...
    fn test_translate_in_place() {
        ...
        assert_eq!(selection, A1Selection::test("B2:C3"));

        // Test negative translation
        let mut selection = A1Selection::test("C3:D4");
        selection.translate_in_place(-1, -1);
        assert_eq!(selection, A1Selection::test("B2:C3"));
        ...
        assert_eq!(selection, A1Selection::test("C1:D2"));

        // Test y-only translation
        let mut selection = A1Selection::test("A1:B2");
        selection.translate_in_place(0, 2);
        assert_eq!(selection, A1Selection::test("A3:B4"));

        // Test single cell selection
        let mut selection = A1Selection::test("A1");
        selection.translate_in_place(1, 1);
        assert_eq!(selection, A1Selection::test("B2"));
    ...
    fn test_translate() {
        // Test positive translation
        let selection = A1Selection::test("A1:B2");
        let translated = selection.translate(1, 1);
        assert_eq!(translated, A1Selection::test("B2:C3"));
        assert_eq!(selection, A1Selection::test("A1:B2"));

        // Test negative translation
        let selection = A1Selection::test("C3:D4");
        let translated = selection.translate(-1, -1);
        assert_eq!(translated, A1Selection::test("B2:C3"));
        assert_eq!(selection, A1Selection::test("C3:D4"));

        // Test zero translation
        let selection = A1Selection::test("A1:B2");
        let translated = selection.translate(0, 0);
        assert_eq!(translated, A1Selection::test("A1:B2"));
        assert_eq!(selection, A1Selection::test("A1:B2"));

        // Test single cell selection
        let selection = A1Selection::test("A1");
        let translated = selection.translate(1, 1);
        assert_eq!(translated, A1Selection::test("B2"));
        assert_eq!(selection, A1Selection::test("A1"));

        // Test negative translation capping
        let selection = A1Selection::test("A1");
        let translated = selection.translate(-10, -10);
        assert_eq!(translated, A1Selection::test("A1"));
        assert_eq!(selection, A1Selection::test("A1"));
    }
}
