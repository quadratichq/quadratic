...
export const useLocalFiles = (sheetController: SheetController): LocalFiles => {
  ...
  const load = useCallback(async (id: string): Promise<GridFileSchemaV1 | undefined> => {
  ...
  const validateFile = useCallback((file: GridFileSchemaV1): boolean => {
    const expected = ['cells', 'formats', 'columns', 'rows', 'borders', 'cell_dependency', 'version', 'modified', 'created', 'id', 'filename'];
   ...
   */
  const loadQuadraticFile = useCallback(async (url: string, overwrite?: boolean): Promise<boolean | 'overwrite'> => {
    ...
    try {
      ...
      if (validateFile(gridFileJSON)) {
        ...
        if (overwrite === undefined && index.find(entry => entry.id === gridFileJSON.id)) {
          return 'overwrite';
        }
      ...
      log('error while fetching file');
      return false;
    }
  }, [index, validateFile]);

  const loadSample = useCallback((sample: string): void => {
  ...
  const newFile = useCallback(async (filename?: string): Promise<void> => {
    if (!index) {
    ...
    const newFile: GridFileSchemaV1 = { ...sheet.export_file(), id: uuid(), created, modified: created, version: VERSION, filename: filename || 'Untitled' };
  ...
  }, [index, lastLoaded, sheet]);

  const saveQuadraticFile = useCallback((autoDownload: boolean): GridFileSchemaV1 | undefined => {
    if (!lastFileContents) return;
    const data: GridFileSchemaV1 = { ...lastFileContents, ...sheet.export_file(), version: VERSION, modified: lastFileContents?.modified };
