...
export const useLocalFiles = (sheetController: SheetController): LocalFiles => {
  ...
  const afterLoad = useCallback((grid: GridFileSchemaV1) => {
  ...
  const newFile = useCallback(async (filename?: string): Promise<void> => {
    ...
    const newFile: GridFileSchemaV1 = { ...grid, id: uuid(), created, version: VERSION, modified: created, filename: filename || 'Untitled' };
