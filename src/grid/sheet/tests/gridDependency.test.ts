...
describe('gridOffsets', () => {
  ...
  it('adds a new cell dependency', () => {
    expect(
      gridRenderDependency.update({ x: 1, y: 2 }, [
        { x: 2, y: 2 },
        { x: 3, y: 2 },
      ])?.length
    ).toBe(2);
    ...
    expect(gridRenderDependency.getChangedCells({ x: 1, y: 2 })).toEqual([
      { x: 2, y: 2 },
      { x: 3, y: 2 },
    ]);
  });
  ...
  it('saves cell dependencies', () => {
    ...
    expect(JSON.stringify(gridRenderDependency.save())).toBe(
      '[{"location":{"x":2,"y":2},"needToRender":[{"x":1,"y":2}],"renderThisCell":[]},{"location":{"x":3,"y":2},"needToRender":[{"x":1,"y":2}],"renderThisCell":[]},{"location":{"x":1,"y":2},"needToRender":[],"renderThisCell":[{"x":2,"y":2},{"x":3,"y":2}]}]'
    );
  });
  ...
  it('gets dependency within bounds', () => {
    ...
    gridRenderDependency.update({ x: 2, y: 2 }, [
      { x: 3, y: 2 },
      { x: 4, y: 2 },
    ]);
