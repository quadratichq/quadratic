...
impl CellRefCoord {
    ...
    pub fn adjust(self, relative_only: bool, delta: i64) -> Result<Self, A1Error> {
        if (relative_only && self.is_absolute) || self.is_unbounded() {
            ...
            match self.coord.saturating_add(delta) {
                ..=0 => Err(A1Error::InvalidCellReference(format!(
                    "Invalid cell reference: {} + {} is out of bounds",
    ...
    fn test_translate() {
        for (init, delta, expected) in [
            ...
            (
                1,
                -1,
                Err(A1Error::InvalidCellReference(format!(
                    "Invalid cell reference: 1 + -1 is out of bounds"
                ))),
            ),
            (10, -1, Ok(9)),
            (UNBOUNDED, -1, Ok(UNBOUNDED)),
            (
                ...
                -999,
                Err(A1Error::InvalidCellReference(format!(
                    "Invalid cell reference: 1 + -999 is out of bounds"
                ))),
            ),
            (
                10,
                -999,
                Err(A1Error::InvalidCellReference(format!(
                    "Invalid cell reference: 10 + -999 is out of bounds"
                ))),
            ),
            ...
            let abs_translated = expected.clone().map(CellRefCoord::new_abs);
