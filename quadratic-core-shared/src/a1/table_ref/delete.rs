...
mod tests {
    use crate::{
        Rect, TableRef,
        a1::{A1Context, A1Selection, CellRefRange, ColRange, RefRangeBounds},
    };

    /// Creates a table context with 3 columns and 3 data rows with name "test_table" at B2
    /// Table bounds: B2:D6 (1 name row + 1 header row + 3 data rows = 5 rows)
    fn setup_3x3_test_table_at_b2() -> (A1Context, TableRef) {
        ...
        let selection = A1Selection::test_a1_context("test_table", &a1_context);
        let CellRefRange::Table { range: table_range } = &selection.ranges[0] else {
            panic!("Expected a table ref");
        };

        (a1_context, table_range.clone())
    }

    #[test]
    fn test_delete_all_from_table_from_selection() {
        let (a1_context, table_range) = setup_3x3_test_table_at_b2();

        let remove_all = A1Selection::test_a1("*");

        let result = table_range.delete(&remove_all.ranges[0], &a1_context);
    ...
    fn test_delete_first_column_from_selection() {
        ...
        let result = table_range.delete(&remove_first_column.ranges[0], &a1_context);
    ...
    fn test_delete_second_column_from_selection() {
        ...
        let result = table_range.delete(&remove_second_column.ranges[0], &a1_context);
    ...
    fn test_delete_first_row_from_selection() {
        ...
        let result = table_range.delete(&remove_first_row.ranges[0], &a1_context);
        assert_eq!(result.len(), 1);
        assert_eq!(
            result[0],
            CellRefRange::Sheet {
    ...
    fn test_delete_table_column_from_selection() {
        let (a1_context, table_range) = setup_3x3_test_table_at_b2();
        let remove_first_column = A1Selection::test_a1_context("test_table[Column 1]", &a1_context);
    ...
    fn test_delete_sheet_from_table_selection() {
        // Table with 2 columns and 2 data rows at B2
        // Table bounds: B2:C5 (1 name row + 1 header row + 2 data rows = 4 rows)
        let a1_context = A1Context::test(
            &[],
            &[(
                "test_table",
        ...
        let to_delete = A1Selection::test_a1_context("B4:C5", &a1_context);
        let result = selection.delete_selection(&to_delete, &a1_context);
