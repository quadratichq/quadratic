...
/// Returns the coordinate of a row from its name, or `None` if no such row exists.
///
/// Row names are simply their numeric value: "1"=1, "2"=2, etc.
pub fn row_from_name(s: &str) -> Option<i64> {
    let row: i64 = s.parse().ok()?;
    if row < 1 {
        return None;
    }
    Some(row)
...
mod tests {
    ...
    #[test]
    fn test_row_from_name() {
        assert_eq!(row_from_name("1"), Some(1));
        assert_eq!(row_from_name("2"), Some(2));
        assert_eq!(row_from_name("100"), Some(100));
        assert_eq!(row_from_name("1000000"), Some(1000000));

        // Invalid inputs
        assert_eq!(row_from_name("0"), None);
        assert_eq!(row_from_name("-1"), None);
        assert_eq!(row_from_name(""), None);
        assert_eq!(row_from_name("A"), None);
        assert_eq!(row_from_name("1A"), None);
        assert_eq!(row_from_name(" "), None);
    }
}
